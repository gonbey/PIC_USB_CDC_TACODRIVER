...
void APP_DeviceCDCEmulatorTasks()
{
    ...
    /** USBデバイスがconfiguredじゃないと何もできないから前のセッツアップに戻る */
    ...
    /** サスペンドだったら、リモートの状態を確認する必要があるよ*/
    if( USBIsDeviceSuspended()== true )
    {
        return;
    }

    /** 前使ったRS232バッファが空であるとき、USBバッファを殻にするよ*/
    ...
    /** UARTから受信文字があるかチェックして、あればUSBに送るよ*/
	if(mDataRdyUSART() && (NextUSBOut < (CDC_DATA_OUT_EP_SIZE - 1)))
    	...
    	//deassert RTS.
        /** RTSピンでもっとデータを送っていいか確認するよ。もし受信バッファがいっぱいだったら否認するよ*/
    ...
    /** 送信キューがないかチェックするよ、もしあって、送り先も大丈夫ならば、USBパケットを送る準備をするよ*/
